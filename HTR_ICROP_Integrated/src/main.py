...
import AHIS_CropModel1 as crop_model
import warnings

# Suppress TensorFlow warnings
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'  # Ignore INFO and WARNING messages

# Suppress any warnings
warnings.filterwarnings("ignore")

# Define the output file path
output_file_path = os.path.join("C:\Users\Owner\Documents\FYP\automated-health-information-system\MDS2_AHIS\data_files", "accuracy.txt")
...
def infer(model: Model, fn_img: Path) -> None:
    ...
    stored_value = recognized[0]
    # Define the output file path
    output_file_path = os.path.join("C:\Users\Owner\Documents\FYP\automated-health-information-system\MDS2_AHIS\data_files", "accuracy.txt")
    # Write the recognized text and probability to a text file
    with open(output_file_path, 'a') as f:  # Use 'a' to append to the file
        f.write(f'Recognized: "{stored_value}", Probability: {probability[0]}\n')
...
def main():
    ...
    try:
        ...
        # infer text on test image
            ...
            #print('entered my infer')
            model = Model(char_list_from_file(), decoder_type, must_restore=True, dump=args.dump)


            # Directory where the cropped images are stored
            #image_dir = '..\data\Form'
            image_dir = r'C:\Users\Owner\Documents\FYP\automated-health-information-system\MDS2_AHIS\data_files\crop_images'
            #image_dir = r'C:\Users\Alicia\OneDrive - Monash University\Desktop\FIT3164\Project\automated-health-information-system\MDS2_AHIS\HTR_Model\SimpleHTR-master\data\Form_UpperCase2'
            
            # Get all PNG files in the directory
            image_files = [f for f in os.listdir(image_dir) if f.endswith('.jpg')]

            # Loop over each image file and run main()
            for img_file in image_files:
                path = os.path.join(image_dir, img_file)
                recognized = infer(model, path)
                data[i] = recognized
                i += 1
                sys.stdout.flush()  # Ensure the data is flushed to stdout immediately
            ...
            patient = {
                ...
                "nationality": data[12],
                "tel": data[13],
                "emgcyRelationship": data[14],
                "state": data[15],
                "postalCode": data[16]
                }
            
            # Define the output file path
            output_file = os.path.join("C:\Users\Owner\Documents\FYP\automated-health-information-system\MDS2_AHIS\data_files", "patient_data.json")

            # Write the dictionary to a JSON file
            with open(output_file, 'w') as f:
                json.dump(patient, f, indent=4)

            print("Patient data successfully written to patient_data.json")
            sys.stdout.flush()  # Ensure the data is flushed to stdout immediately
            #return patient
